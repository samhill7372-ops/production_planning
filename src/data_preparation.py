...
def load_precomputed_historical_data(year: str = None) -> Optional[Dict]:
    """
    Load pre-computed historical data from joblib files.

    These files are generated by scripts/prepare_deployment.py and contain
    all the statistics needed for prediction without raw CSV files.

    Args:
        year: Year to load data for (e.g., '2024', '2025').
              If None, tries to load from the most recent year.

    Returns:
        Dict with:
        - 'material_stats': Material-level statistics for forward prediction
        - 'kd_lookup': KD distribution lookup by input material
        - 'summary_df': Historical summary DataFrame
        - 'generated_at': Timestamp when file was generated
    ...
    if year is None:
        ...
        for y in ['2025', '2024', '2023']:
            precomputed_path = os.path.join(MODELS_DIR, y, "historical_data_precomputed.joblib")
            if os.path.exists(precomputed_path):
                year = y
                break

    if year is None:
        print("No pre-computed historical data found")
        return None

    precomputed_path = os.path.join(MODELS_DIR, year, "historical_data_precomputed.joblib")

    if not os.path.exists(precomputed_path):
        print(f"Pre-computed file not found: {precomputed_path}")
        return None

    try:
        data = joblib.load(precomputed_path)
        print(f"Loaded pre-computed historical data for {year}")
        print(f"  Generated at: {data.get('generated_at', 'unknown')}")
        print(f"  Material stats: {len(data.get('material_stats', {}))} materials")
        print(f"  KD lookup: {len(data.get('kd_lookup', {}))} input materials")
        return data
